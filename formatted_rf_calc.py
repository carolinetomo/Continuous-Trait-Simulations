...
rfout.write("trait_type\tnum_traits\trate\tunweighted_rf\tweighted_rf\teuclidean_dist\n")
...
for i in os.listdir(itdir):
    ...
    for j in os.listdir(i):
        if "sumtree" in j:#"mcc" in j  and j.split(".")[-1]!="rr":#j.split(".")[-2] =="mcc" or j.split(".")[-3]=="mcc": #j.split(".")[-1] == "tre" or j.split(".")[-1] == "sumtree":
            spls = j.split(".")
            num = spls[0]
            tree = dendropy.Tree()
            ...
            rfout.write(trait_cat+"\t"+str(aln_size)+"\t"+str(rate)+"\t"+str(treecompare.symmetric_difference(tt,it))+"\t"+str(treecompare.weighted_robinson_foulds_distance(tt,it))+"\t"+str(treecompare.euclidean_distance(tt,it))+"\n")
